  ...
  public Stream<CompletableFuture<String>> findPricesStream(String product) {
    return shops.stream()
        .map(shop -> CompletableFuture
            .supplyAsync(() -> shop.getPrice(product), executor)) //1、以异步方式取得每个shop中指定产品的原始价格
        .map(future -> future.thenApply(
            Quote::parse))//2、Quote 对象存在时，对 其返回的值进行转换。将 Stream 中的每个 CompletableFuture<String> 对象转换为对应的CompletableFuture<Quote> 对象
        .map(future -> future.thenCompose(quote -> CompletableFuture
            .supplyAsync(() -> Discount.applyDiscount(quote),
                executor)));//3、使用另一个异步任务构造期望的 Future申请折扣
  }

  /**
   * thenAccept：定义了如何处理 CompletableFuture 返回的结果，一旦CompletableFuture 计算得到结果，它就返回一个
   * CompletableFuture<Void> 。 故意抛异常，能获取到多少算多少的result
   */
