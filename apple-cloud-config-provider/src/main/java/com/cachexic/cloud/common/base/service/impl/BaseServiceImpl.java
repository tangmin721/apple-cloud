 ...
 */
public abstract class BaseServiceImpl<T extends PojoBaseEntity, Q extends PojoBaseQuery> implements
    BaseService<T, Q> {
  ...
  public Long insert(T entity) {
    ...
    if (!validateResult.getFlag()) {
      throw new ValidBizException(ValidBizExceptionEnum.VALID_INSERT_ERROR.getCode(),
          JsonUtil.toJson(validateResult.getErrorObjs()));
  ...
  public int update(T entity) {
    ...
    if (!validateResult.getFlag()) {
      throw new ValidBizException(ValidBizExceptionEnum.VALID_UPDATE_ERROR.getCode(),
          JsonUtil.toJson(validateResult.getErrorObjs()));
  ...
  public Pagination<T> selectListPagination(Q query) {
    ...
    Pagination<T> pagination = new Pagination<T>(query.getCurrentPage(), query.getPageSize(),
        total);
    if (total > 0) {
